    ...
    <section className="w-full py-12 sm:py-16 lg:py-24 bg-slate-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="bg-yellow-50 border border-yellow-200 rounded-lg sm:rounded-xl p-3 sm:p-4 mb-8 sm:mb-12 max-w-2xl mx-auto"
        >
        ...
        <div className="grid grid-cols-3 gap-3 sm:gap-6 mb-8 sm:mb-12">
          {stats.map((stat, index) => (
            <motion.div
              ...
              className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 text-center border border-gray-100 shadow-sm"
            >
              <div className="w-10 h-10 sm:w-14 sm:h-14 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-2 sm:mb-4">
                <stat.icon className="w-5 h-5 sm:w-7 sm:h-7 text-green-600" />
              </div>
              <p className="text-2xl sm:text-4xl font-bold text-gray-900 mb-0.5 sm:mb-1">{stat.value}</p>
              <p className="font-semibold text-gray-700 text-xs sm:text-base">{stat.label}</p>
              <p className="text-[10px] sm:text-sm text-gray-500 mt-0.5 sm:mt-1">{stat.desc}</p>
            </motion.div>
          ))}
        </div>
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          transition={{ duration: 0.6 }}
          className="bg-white rounded-xl sm:rounded-2xl p-5 sm:p-8 border border-gray-100 shadow-sm"
        >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
            {[
              ...
              <div key={i} className={`${item.bgColor} rounded-lg sm:rounded-xl p-3 sm:p-4 text-center`}>
                <div className={`w-3 h-3 sm:w-4 sm:h-4 ${item.color} rounded-full mx-auto mb-1.5 sm:mb-2`} />
                <p className={`font-bold text-xs sm:text-sm ${item.textColor}`}>{item.day}</p>
                <p className="text-[10px] sm:text-xs text-gray-600 mt-0.5 sm:mt-1">{item.status}</p>
              </div>
            ))}
          </div>
        </motion.div>
